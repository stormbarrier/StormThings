 ...
 */
metadata {
	definition (name: "Z-Wave Lock Reporting (stormbarrier)", namespace: "stormbarrier", author: "SmartThings") {
		capability "Actuator"
		capability "Lock"
		capability "Polling"
		capability "Refresh"
...
def updated() {
	try {
		if (!state.init) {
            ...
            log.debug "updated() called. Asking for doorLock and battery info."
		}
	} catch (e) {
...
def zwaveEvent(physicalgraph.zwave.commands.batteryv1.BatteryReport cmd) {
	// Modified on 9/20/2017 to add log.debug info
    log.debug "Current state.lastbatt: $state.lastbatt"
	...
	state.lastbatt = now()
	// Modified on 9/20/2017 to add log.debug info
    log.debug "Updated state.lastbatt: $state.lastbatt"
	createEvent(map)
}
...
def refresh() {
	...
	// Changed on 9/20/2017 - wanted to see if adding .batteryGet() to the cmds worked. It did. Which tells me the updated() function isn't running.
